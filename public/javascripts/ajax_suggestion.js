...
var ajaxSuggestions = {
    ...
    valueAddedFromResultsListToInput: true,//false
    init: function(){
        this.xmlHttp = this.createXmlHttp();
        if (this.xmlHttp) {
            this.elements = $('.ajax-suggestion');
			this.applyEvents();
    ...
    applyEvents: function(){
        ...
        for (var i = 0; i < this.elements.length; i++) {
			...
			this.addEvent(element, "blur", this.updateBlurResults);
            if (this.hideResultsOnDocumentClick) {
	...
	updateBlurResults: function(evt){ 
		setTimeout("ajaxSuggestions.clearBlurResults()", ajaxSuggestions.timeBeforeSuggest);
    },
	
	clearBlurResults: function(){
		if(this.currentKeyEvent!=40){
		   ajaxSuggestions.clearResults();
		   if(jQuery('#user_password')){
    ...
    getSuggestions: function(){
        ...
        if (!/13|27|37|39/.test(this.currentKeyEvent)) {
            ...
            var class_names = this.currentElm.className.split(' ');
            var url = class_names[class_names.length - 1];
            ...
            if (value.length > this.charactersBeforeSearch && url.length > 0) {
                ...
                if (value.length == 0 || !this.valueAddedFromResultsListToInput) {
                    ajaxSuggestions.clearResults();
                }
        }
    },
    ...
    makeSuggestionCall: function(value, url){
        ...
        if (!exists) {
          ...
          //  new Ajax.Request(url, {
           //     method: 'get',
            //    onComplete: this.getAjaxResults
            //});
    ...
    navigateResults: function(evt, keyCode){
        if (ajaxSuggestions.currentElm && ajaxSuggestions.elmToPresentResultsIn) {
            ...
            var selectedItem = ajaxSuggestions.selectedItem;
            if (!evt && keyCode == 38) {
    ...
    insertValueIntoField: function(evt){
        ...
        var value = elm.getAttribute("title");
