...
MzTreeNode.prototype.upCheck = function()
{
	...
	if(node.parentNode)
	{
		var BrotherNode, BrotherPic;
		var BrotherHalfCheck = false;
		for(var i=0; i<node.parentNode.childNodes.length; i++)
		{
			BrotherNode = node.parentNode.childNodes[i];
			if (BrotherPic = document.getElementById(mtv.index+"_checkbox_"+BrotherNode.index))
			{
				if (BrotherPic.src == MzTreeView.icons.line["c2"].src){
					BrotherHalfCheck = true;
				}
			}		
			if (BrotherNode.checked != node.checked || BrotherHalfCheck) //°ëÑ¡×´Ì¬checken==false
			{
