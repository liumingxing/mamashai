...
//	{   
//		var me=a.nodes[i], mtv=me.$$caller
//		var chk; 
//		if(chk=document.getElementById(mtv.index+"_checkbox_"+ me.index))
//		{
//			if(chk.src == MzTreeView.icons.line["c1"].src ||chk.src == MzTreeView.icons.line["c2"].src)
//			{
//				if(me.hasChild && !me.isLoaded)
//				{
//					me.expand();
//				}
//				GetCheckID = GetCheckID + ", " + me.id;
//			}
//		}
//	}
//	return GetCheckID;
...
function LoadCheckBox(a,ArrayName)
{   
	var ArrayCheckedID = ArrayName;
  expandWithCheckArray(a,ArrayName);
	for(var i in a.nodes)
	{ 
		var me=a.nodes[i], mtv=me.$$caller
		var chk; 
		if(chk=document.getElementById(mtv.index+"_checkbox_"+ me.index))
		{
			for (var j=0;j<ArrayCheckedID.length ;j++ )
			{
				if(me.id==ArrayCheckedID[j]){
					me.checked = true;
					chk.src = MzTreeView.icons.line["c1"].src;
					me.upCheck();
				}
			}
		}
...
// 根据以经有的节点数组去展开一棵树
function expandWithCheckArray(a,ArrayName)
{   
	var ArrayCheckedID = ArrayName;
	for(var i in a.nodes)
	{ 
