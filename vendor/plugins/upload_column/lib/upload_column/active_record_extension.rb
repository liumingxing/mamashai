...
module UploadColumn
  ...
  module ActiveRecordExtension
    ...
    def self.append_features(base) #:nodoc:
     ...
     # add by jerry 
      base.before_destroy :destroy_uploaded_files
    end
    ...
    def save_uploaded_files
      if @files
        @files.each { |k, v| 
          v.send(:destroy) if v and v.tempfile? 
          v.send(:save) if v and v.tempfile? 
        }
      end
    end
    ...
    def destroy_uploaded_files
      FileUtils.rm_rf File.join(UploadColumn.configuration[:root_dir], "#{self.class.name.downcase}","#{self.id}")
    end
