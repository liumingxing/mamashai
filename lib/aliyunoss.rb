...
=end
      def put(path, file, options={})
          ...
          #"Content-Disposition" => "inline;filename=#{path.split('/').last}",
            
          new_unsafe =  Regexp.union [URI::Parser.new.regexp[:UNSAFE],'~','@', '[', ']']
          RestClient.put(URI.encode(url, new_unsafe), file, headers)
